...
obodc 有集中化部署的 **Web 版**和本地运行的**客户端版本。**Web 版本需要依赖 obodc 后端运行，客户端版本会自动安装对应的依赖，并生成一个可独立运行的安装包。
...
打包完 obodc Server Jar 包之后，需要在如下目录存放 Jar 包。[如何构建 Jar](../../docs/zh-CN/DEVELOPER_GUIDE.md)
...
修改 `config/config.js` 中的 `proxy` 字段，将其中的 `target` 属性**都更改**为 obodc Server 的地址。

```shell
proxy: {
    '/api/v1/webSocket/obclient': {
      target: 'obodc Server 地址',
      ws: true,
    },
    '/api/': {
      target: 'obodc Server 地址',
...
此时会在默认的 `8000` 端口建立一个 web 服务器，访问[ http://localhost:8000](https://localhost:8000) 即可打开 obodc。

#### 客户端开发

obodc 客户端基于 Electron 来开发，我们需要启动 Web 服务与 Electron 服务两个服务。
