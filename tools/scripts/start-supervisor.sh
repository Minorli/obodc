...
bin_parent_directory=$(dirname "$bin_directory")
if [ "$(basename "$bin_directory")" = "scripts" ] && [ "$(basename "$bin_parent_directory")" = "tools" ]; then
    install_directory=$(dirname "$bin_parent_directory")
else
    install_directory="$bin_parent_directory"
fi
